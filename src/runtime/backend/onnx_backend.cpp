...
Device ResultDevice(const OnnxBackendSessionOptions& options) {
  if (options.backend == OnnxBackendKind::kGpu) {
    return Device{DeviceType::kCuda, options.device_id};
  }
  return Device{DeviceType::kCpu, 0};
}
...
std::vector<std::string> OutputNamesFromModelInfo(const ModelInfo& model_info) {
...
bool NeedsIoBinding(const std::vector<const Tensor*>& inputs,
  ...
  if (result_device.type == DeviceType::kCuda) {
    return true;
  }
...
class OnnxBackendSession {
 public:
  OnnxBackendSession(OnnxBackendSessionOptions options, const Model& model,
                     ModelInfo& active_info)
    ...
    active_info =
        ResolveActiveModelInfo(ReadModelInfo(session_), options_.output_names);
    output_names_ = OutputNamesFromModelInfo(active_info);
    input_names_.reserve(active_info.inputs.size());
    for (const TensorInfo& input : active_info.inputs) {
  ...
  std::vector<Tensor> Infer(const std::vector<Tensor>& inputs,
    ...
    const Device result_device = ResultDevice(options_);

    if (NeedsIoBinding(ordered_inputs, result_device)) {
 ...
 private:
  std::vector<const Tensor*> ResolveInputs(const std::vector<Tensor>& inputs,
    ...
    if (inputs.size() == 1 && inputs.front().name().empty()) {
      ValidateInputTensor(inputs.front(), model_info.inputs.front(), options_);
    ...
    for (const Tensor& input : inputs) {
      ...
      ValidateInputTensor(input, model_info.inputs[index], options_);
      ordered[index] = &input;
      assigned[index] = true;
    }
  ...
  std::vector<Tensor> InferWithIoBinding(
      ...
      std::vector<Ort::Value> input_values, Device result_device) {
    Ort::IoBinding binding(session_);
    for (std::size_t index = 0; index < inputs.size(); ++index) {
      binding.BindInput(input_names_[index].c_str(), input_values[index]);
    }

    Ort::MemoryInfo output_memory_info = MakeMemoryInfo(result_device);
  ...
  std::vector<Tensor> WrapOutputs(std::vector<Ort::Value> output_values) const {
    ...
    for (std::size_t index = 0; index < output_values.size(); ++index) {
      ...
      TensorDesc desc;
      desc.info.name = output_names_[index];
      desc.info.data_type = FromOrtDataType(tensor_info.GetElementType());
 ...
 public:
  bool Supports(const Model& model, Device execution_device) const override {
    return model.format == ModelFormat::kOnnx &&
           ...
           execution_device.id >= 0 && IsOnnxGpuBackendAvailableImpl();
  }
...
std::unique_ptr<BackendAdapter> CreateOnnxCpuBackendAdapter() {
  return std::make_unique<OnnxCpuBackendAdapter>();
}

#if defined(MW_INFER_HAS_ONNXRUNTIME_CUDA_PROVIDER)
std::unique_ptr<BackendAdapter> CreateOnnxGpuBackendAdapter() {
  return std::make_unique<OnnxGpuBackendAdapter>();
}
#endif
