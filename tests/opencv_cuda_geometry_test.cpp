...
TEST(OpenCvCudaGeometryTest, ResizesAndPadsFourChannelGpuMat) {
  ...
  batch = Resize(std::move(batch), ImageSize{5, 5}, Interpolation::kNearest);
  batch = Pad(std::move(batch), Padding{1, 2, 3, 4}, FillValue{{1, 2, 3, 4}});
...
TEST(OpenCvCudaGeometryTest, RejectsSixAndNineChannelGpuMatPad) {
...

  {
    ...
    EXPECT_THROW(
        static_cast<void>(Pad(raw_images, Padding{1, 1, 1, 1}, FillValue{})),
        std::invalid_argument);
  }
  {
    RawImageBatch raw_images(std::vector<cv::cuda::GpuMat>{
