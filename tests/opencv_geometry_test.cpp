...
TEST(OpenCvGeometryTest, ResizesRawImageBatchAndRestoresPoint) {
  ...
  const cv::Mat& output = GetOpenCvMat(resized.frame(0).image);
...
TEST(OpenCvGeometryTest, LetterBoxesImageAndRecordsSingleStep) {
  ...
  const cv::Mat& output = GetOpenCvMat(boxed.frame(0).image);
  EXPECT_EQ(output.cols, 40);
  EXPECT_EQ(output.rows, 40);
  EXPECT_EQ(output.at<cv::Vec3b>(0, 0), cv::Vec3b(7, 7, 7));

  const GeometryTrace& trace = boxed.frame(0).geometry_trace;
  ASSERT_EQ(trace.size(), 1U);
...
TEST(OpenCvGeometryTest, CropsThenPadsImageBatch) {
  ...
  const cv::Mat& output = GetOpenCvMat(batch.frame(0).image);
...
TEST(OpenCvGeometryTest, CenterCropsEachFrame) {
  ...
  ImageBatch cropped = CenterCrop(raw_images, ImageSize{8, 6});

  const cv::Mat& output = GetOpenCvMat(cropped.frame(0).image);
  EXPECT_EQ(output.cols, 8);
  EXPECT_EQ(output.rows, 6);

  const GeometryStep& step = cropped.frame(0).geometry_trace.step(0);
