...
TEST(CudaTensorTest, AllocatesCudaTensor) {
  if (!IsCudaTensorAllocationAvailable()) {
    GTEST_SKIP() << "CUDA tensor allocation is unavailable";
  }

  Tensor tensor = AllocateCudaTensor(MakeCudaDesc());
...
TEST(CudaTensorTest, TensorInterfaceSelectsCudaAdapterByDevice) {
  if (!IsCudaTensorAllocationAvailable()) {
    GTEST_SKIP() << "CUDA tensor allocation is unavailable";
  }

  Tensor tensor = Tensor::Allocate(MakeCudaDesc(), MakeCudaTensorAllocator());
...
TEST(CudaTensorTest, TensorBufferReusesAndGrowsLikeVectorReserve) {
  if (!IsCudaTensorAllocationAvailable()) {
    GTEST_SKIP() << "CUDA tensor allocation is unavailable";
  }

  TensorBuffer buffer(MakeCudaTensorAllocator());
...
TEST(CudaTensorTest, TensorBufferSelectsExplicitCompositeAdapterByDevice) {
  if (!IsCudaTensorAllocationAvailable()) {
    GTEST_SKIP() << "CUDA tensor allocation is unavailable";
  }

  TensorBuffer buffer(TensorAllocator(
      {&GetHostTensorAllocationAdapter(), &GetCudaTensorAllocationAdapter()}));
...
TEST(CudaTensorTest, RejectsInvalidCudaAllocationRequests) {
  TensorDesc desc = MakeCudaDesc();
  desc.device = Device{DeviceType::kCpu, 0};
  EXPECT_THROW(static_cast<void>(AllocateCudaTensor(desc)),
  ...
  TensorAllocator allocator = MakeCudaTensorAllocator();
  desc = MakeCudaDesc();
  desc.device = Device{DeviceType::kCpu, 0};
